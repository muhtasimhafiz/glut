     ...
     for(int j=0;j<eneYpos.size();j++)
            {
                ...
                if(abs(bulletYPos[i]-eneYpos[j])<=bulletsize/2 &&abs(bulletXpos[i]-eneyXpos[j])<=bulletsize/2)
                {
...
void bossHitDetect()
{
        ...
        for(int i=0;i<bulletXpos.size();i++)
        {
             if(abs(bulletYPos[i]-(3))>=0&&abs(bulletYPos[i]-(3))<=bulletsize && abs(bulletXpos[i]-trans)>=0&&abs(bossBulletX[i]-trans)<=bulletsize && !gameover)
         {
                cout<<" boss hit"<<endl;
                score++;
                bulletYPos.erase(bulletYPos.begin() + i);;
...
void update(int value) {
    ...
    {
            if(score>150 && boss ==false && transition==false)
            {
